...
import { AppConfig, APIClient, PlayerStatistics, OrderPlayerBaseStats } from "../../libs/api";
...
export class To4stPlayerStatistics implements ComponentInterface {
   ...
   * Content
   */
  @State() playerstats = [] as PlayerStatistics[];
  ...
  columns = [
    {
      ...
      tableContent: player =>  <a target="_blank" href={`http://steamcommunity.com/profiles/${player.steamId64}`}><img src={player.steamUser?.avatarMediumUrl} width="48" height="48" /></a> ,
    },
    {
      name: "SteamId64",
  ...
  async updateContent()
  {
    if((this.isAdmin || this.appConfig?.publicStats) && !!this.apiClient)
    {
      ...
      try 
      {
        const res = await this.apiClient.client.chain.query.playerStatistics({options: {pageSize: 25, page: this.currentPage, orderDesc: this.orderDesc, orderBy: this.currentOrderBy, ranked: true, cachedIfPossible: true}}).execute(
          {
            pageCount: true, 
            content: 
            {
        ...
        this.allImagesValid = this.playerstats.every(x => x.steamUser?.avatarMediumUrl);
        this.hasValidNames = this.allNamesValid || this.playerstats.some(x => x.steamUser?.name);
