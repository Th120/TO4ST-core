...
export class To4stMatchConfigList implements ComponentInterface {
  ...
  @State() columns = [
    ...
    },
    {
      ...
      detailInput: (item, cb) => (
        <input
          ...
          class="input"
          onChange={(event) =>
            cb("configName", (event.target as HTMLInputElement).value.trim())
          }
        />
      ),
      input: (item, cb) => (
      ...
      detailInput: (item, cb) => (
        <div class="select">
          <select
            onChange={(e) =>
              cb("gameMode", {
                name: (e.target as HTMLSelectElement).value.trim(),
              })
            }
          >
            {this.gameModes.map((x) => (
              <option selected={item?.gameMode?.name === x.name}>
                {x.name}
              </option>
            ))}
          </select>
    ...
    },
    {
      ...
      detailInput: (item, cb) => (
        <input
          type="number"
          placeholder="Matchend Length"
          min="1"
          max="60"
          value={item?.matchEndLength ?? 0}
          class="input"
          onChange={(event) =>
            cb(
              "matchendLength",
    ...
    },
    {
      ...
      detailInput: (item, cb) => (
        <input
          type="number"
          placeholder="Warm Up Length"
          min="1"
          max="300"
          value={item?.warmUpLength ?? 0}
          class="input"
          onChange={(event) =>
            cb(
              "warmUpLength",
    ...
    },
    {
      ...
      detailInput: (item, cb) => (
        <input
          ...
          max="100"
          step="0.5"
          value={item?.friendlyFireScale ?? 0}
          class="input"
          onChange={(event) =>
            cb(
              "friendlyFireScale",
    ...
    },
    {
      ...
      detailInput: (item, cb) => (
        <input
          type="number"
          placeholder="Round End Length"
          min="1"
          max="20"
          value={item?.roundEndLength ?? 0}
          class="input"
          onChange={(event) =>
            cb(
              "roundEndLength",
    ...
    },
    {
      ...
      detailInput: (item, cb) => (
        <to4st-switch
          value={item?.allowGhostcam}
          onToggle={(event) => cb("allowGhostcam", event.detail)}
        />
      ),
    },
    {
    ...
    },
    {
      ...
      detailInput: (item, cb) => (
        <to4st-switch
          value={item?.autoBalanceTeams}
          onToggle={(event) => cb("autoBalanceTeams", event.detail)}
        />
      ),
      shouldBeVisible: () => false,
    ...
    },
    {
      ...
      detailInput: (item, cb) => (
        <to4st-switch
          value={item?.playerVoteTeamOnly}
          onToggle={(event) => cb("playerVoteTeamOnly", event.detail)}
        />
      ),
      shouldBeVisible: () => false,
    },
    {
      ...
      detailInput: (item, cb) => (
        <input
          type="number"
          placeholder="Max Team Damage"
          min="0"
          max="99999"
          value={item?.maxTeamDamage ?? 0}
          class="input"
          onChange={(event) =>
            cb(
              "maxTeamDamage",
    ...
    },
    {
      ...
      detailInput: (item, cb) => (
        <to4st-switch
          value={item?.enablePlayerVote}
          onToggle={(event) => cb("enablePlayerVote", event.detail)}
        />
      ),
    ...
    },
    {
      ...
      detailInput: (item, cb) => (
        <to4st-switch
          value={item?.autoSwapTeams}
          onToggle={(event) => cb("autoSwapTeams", event.detail)}
        />
      ),
      tooltip: () =>
        "If a roundlimit is used the players swap teams after half of the rounds",
    },
    {
      ...
      detailInput: (item, cb) => (
        <input
          type="number"
          placeholder="Mid Game Break Length"
          min="0"
          max="300"
          value={item?.midGameBreakLength ?? 0}
          class="input"
          onChange={(event) =>
            cb(
              "midGameBreakLength",
      ...
      tooltip: () =>
        "If a roundlimit is used the game pauses for given seconds when half of the rounds are over",
    },
    {
      ...
      detailInput: (item, cb) => (
        <to4st-switch
          value={item?.nadeRestriction}
          onToggle={(event) => cb("nadeRestriction", event.detail)}
        />
      ),
    ...
    },
    {
      ...
      detailInput: (item, cb) => (
        <to4st-switch
          value={item?.globalVoicechat}
          onToggle={(event) => cb("globalVoicechat", event.detail)}
        />
      ),
    },
    {
      ...
      detailInput: (item, cb) => (
        <to4st-switch
          value={item?.muteDeadToTeam}
          onToggle={(event) => cb("muteDeadToTeam", event.detail)}
        />
      ),
    },
    {
      ...
      detailInput: (item, cb) => (
        <to4st-switch
          value={item?.ranked}
          onToggle={(event) => cb("ranked", event.detail)}
        />
      ),
    ...
    },
    {
      ...
      detailInput: (item, cb) => (
        <to4st-switch
          value={item?.private}
          onToggle={(event) => cb("private", event.detail)}
        />
      ),
      tooltip: () => "Server requires password or reserved slot access",
    },
  ] as ColumnDetailProps<TMatchConfig>[];

  async componentWillLoad() {
    (async () => {
      const [found] = await GameserverService.get(this.apiClient).gamemodes();

      this.gameModes = found;
  ...
  async saveMatchConfig(
    ...
    let o = entity as TMatchConfig;

    if (!o.id) {
      // create only inits with a name
    ...
    try {
      ...
      await GameserverService.get(this.apiClient).createUpdateMatchConfig(
        {
          id: o.id,
  ...
  async removeMatchConfig(entity: any, onDeletedEntity: () => void) {
    const o = entity as TMatchConfig;
    try {
      await GameserverService.get(this.apiClient).deleteMatchConfig(o.id);
    } catch (e) {
      console.error(e);
    }
  ...
  async updateMatchConfigs(
    ...
    try {
      const [res, pageCount] = await GameserverService.get(
        this.apiClient
      ).matchConfigs(page, PAGE_SIZE, search, orderDesc);

      this.matchConfigs = res;

      onFetchedData(res, pageCount);
    } catch (e) {
      console.error(e);
      onFetchedData([], 1);
    }
    ...
    return (
      <Host>
        <to4st-details
          ...
          hasSearch={true}
          mapPreSerializeEntity={(item: TMatchConfig) => {
          ...
          onSaveEntity={(e) =>
            this.saveMatchConfig(
              e.detail.entity as TMatchConfig,
              e.detail.transactionId,
              e.detail.afterEx
            )
          }
          onDeleteEntity={(e) =>
            this.removeMatchConfig(
              e.detail.entity as TMatchConfig,
